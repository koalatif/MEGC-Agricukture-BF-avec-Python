...
def _labels(m):
    import openpyxl
    p="/sessions/relaxed-adoring-noether/mnt/Modèles MEGC_PEP/MCS_2018_ajusté_vf.xlsx"
    if os.path.exists(p):
        ws=openpyxl.load_workbook(p,data_only=True).active
        lab={str(ws.cell(r,2).value).strip():str(ws.cell(r,1).value).strip() for r in range(4,ws.max_row+1) if ws.cell(r,2).value}
        return lab
    return {}
...
def macro_table(m, r0, r1=None):
    def g(r): return dict(PIB_VA=r['GDP_VA']/1e3, Conso=r['Conso']/1e3, Invest=r['Invest']/1e3,
        ...
        Recettes_pub=r['GVTrev']/1e3, Salaire_moy=float(np.mean(r['W'])))
    base=g(r0)
