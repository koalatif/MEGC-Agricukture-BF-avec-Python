...
def solve_homotopy(m, shock, x0, steps=5):
    """Résolution robuste: applique le choc progressivement (0->1), réchauffage sans repli."""
    from scipy.optimize import root
    x=x0.copy(); sol=None
    for sc in np.linspace(1.0/steps,1.0,steps):
        shock(m,0,sc)
        sol=root(m.residual,x,method='lm'); x=sol.x
    return x,sol

def run_static(shock, sec_cap=False, closure=None, steps=6, verbose=True):
    ...
    xb,_=m.solve(); m.Wbar=m._store['W'].copy(); r0=m.report(xb)
    m2=CGE(); m2.sec_cap=sec_cap; _apply_closure(m2,closure); m2.Wbar=m.Wbar
    x2,sol=(solve_homotopy(m2,shock,xb,steps) if shock else (xb,None))
    r1=m2.report(x2)
...
def run_dynamic(shock=None, T=10, sec_cap=False, verbose=True):
