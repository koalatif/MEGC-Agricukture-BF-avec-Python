...
class CGE:
    ...
    def solve(self,x0=None,method='lm',tol=1e-9,warn=True,log_vars=None):
        ...
        return best.x,best
    def solve_path(self,setter,x0=None,step0=0.15,tol=1e-8,max_time=600,verbose=False):
        ...
        while cur<1.0-1e-12:
            ...
            s=min(cur+step,1.0)
            setter(self,s)
            sol=root(res_piv,x,method='lm',options={'maxiter':getattr(self,'path_maxiter',20000)})
